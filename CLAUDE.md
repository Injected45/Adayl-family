...
`supabase/RESET_AND_APPLY.sql` is the same bundle for a project that already has
a schema. `APPLY_TO_SUPABASE.sql` cannot convert one — `CREATE TYPE` has no
`IF NOT EXISTS`, so the first enum aborts with 42710 — so this file empties
`public` and rebuilds it. Both are generated by `bundle.sh` from one body, and it
refuses to write them if the two bodies differ. It **destroys every row in
`public`**; `auth.users` survives, so logins keep working while every role and
approval is gone with `profiles` and `bootstrap_first_admin.sql` has to be run
again. A preflight block refuses outright if any extension lives in `public`,
because `DROP SCHEMA … CASCADE` would take it too.
