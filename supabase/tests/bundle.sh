...
# ---------------------------------------------------------------------------
#  ⛔ NOT GENERATED BY DEFAULT, as of 2026-08-16.
#
#  This is the only file in the repository that can destroy the association's
#  project, and on 2026-08-16 it did: pasted into the live project, it dropped
#  every عديل, every receipt, and `public.profiles` — which took every role and
#  approval with it and locked the association out of its own app. The data
#  guard inside it did not fire, because it counted RECORDS and the ledger had
#  not started yet.
#
#  Both holes are fixed (profiles is counted first now), but the association's
#  instruction is simpler than a guard: the file is not to exist in the folder
#  they paste from. A file that is not there cannot be pasted by mistake, and
#  no warning has ever been as reliable as absence.
#
#  The capability is kept, behind a deliberate command:
#
#      bash supabase/tests/bundle.sh --with-reset
#
#  Without the flag, any RESET_AND_APPLY.sql lying around is DELETED, so a copy
#  generated before today cannot survive as a trap.
RESET="$HERE/../RESET_AND_APPLY.sql"

WITH_RESET=0
for arg in "$@"; do
  [ "$arg" = "--with-reset" ] && WITH_RESET=1
done

if [ "$WITH_RESET" -eq 0 ]; then
  if [ -f "$RESET" ]; then
    rm -f "$RESET"
    echo "removed $RESET  (destructive; pass --with-reset to generate it)"
  else
    echo "skipped RESET_AND_APPLY.sql  (destructive; pass --with-reset to generate it)"
  fi
  rm -f "$RESET.body"
  exit 0
fi

echo "⚠ --with-reset: generating the file that DROPS SCHEMA public CASCADE"
...
BEGIN
  FOREACH v_tbl IN ARRAY ARRAY[
    -- `profiles` FIRST, and it is the one that was missing on 2026-08-16.
    --
    -- The list below counted RECORDS — عدايل, receivables, payments — so a
    -- project whose ledger had not started yet counted ZERO and the guard did
    -- not fire. That is precisely the project this file was pasted into: the
    -- schema was applied, nobody had been billed anything, so nothing here
    -- objected, DROP SCHEMA ran, and `profiles` went with it. auth.users
    -- survived, so signing in still "worked" — and then found no row, no role
    -- and no approval. The association was locked out of its own app by a file
    -- that had just reported nothing to lose.
    --
    -- profiles is not data, it is ACCESS. One row in it is reason enough to
    -- refuse, because rebuilding it means running bootstrap_first_admin.sql
    -- again and re-approving every member by hand.
    'profiles',
  ...
  IF v_rows > 0 AND v_confirm IS DISTINCT FROM v_phrase THEN
    RAISE EXCEPTION
      'REFUSING TO RESET: this project holds % row(s) (%). Nothing has been '
      'dropped. If `profiles` is in that list, running this file LOCKS THE '
      'ASSOCIATION OUT: auth.users survives so sign-in still succeeds, but '
      'every role and approval is gone and there is no admin left to grant '
      'them — bootstrap_first_admin.sql has to be run again and every member '
      're-approved by hand. Take a backup from Database -> Backups FIRST (the '
      'free tier has none, so export what you need), then set v_confirm at the '
      'top of this block to the phrase named beside it and run the file again.',
