...
SELECT * FROM (
  ...
  UNION ALL SELECT 8, 'عدايل مرتبطون بالبوابة (نطاق أثر قفل الجهاز)',
         ...
         END FROM have
  UNION ALL SELECT 9, 'السجل',
         CASE WHEN NOT adeel_schema THEN 'unknown'
              ELSE coalesce(n_adeels, 0)::text || ' عديل، '
                || coalesce(n_payments, 0)::text || ' إيصال' END FROM have
  UNION ALL SELECT 10, '‹‹ VERDICT ››',
         CASE WHEN NOT adeel_schema
                THEN 'STOP — this is not the adeel schema. Do NOT apply the patch.'
              WHEN NOT patch_0816
                THEN 'STOP — PATCH_20260816_payer_bank_details.sql is missing. Apply that first.'
              WHEN device_lock AND money_out AND spend_rpc AND assoc_finance
                THEN 'PATCH 17/08 is ALREADY applied. Nothing to do.'
              ELSE 'READY — apply supabase/PATCH_20260817_device_lock.sql'
         END FROM have
) t ORDER BY ord;
